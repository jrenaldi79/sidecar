/**
 * SVG icons for tool types and file extensions.
 */
const TOOL_ICONS = {
...
const DEFAULT_ICON = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>';

// Also expose default icon under _default key for tool group headers
TOOL_ICONS['_default'] = DEFAULT_ICON;

/**
 * Returns an SVG icon string for the given tool name.
 * @param {string} tool - Tool name (case-insensitive).
 * @returns {string} SVG markup string.
 */
function getToolIcon(tool) {
  return TOOL_ICONS[tool.toLowerCase()] || DEFAULT_ICON;
}
...
/**
 * Returns a badge HTML string for the given file extension.
 * @param {string} ext - File extension without the dot (e.g. 'js', 'ts').
 * @returns {string} HTML span string.
 */
function getFileIcon(ext) {
  return FILE_ICONS[ext] || '<span class="file-icon file-icon-default"></span>';
}

module.exports = { getToolIcon, getFileIcon };
